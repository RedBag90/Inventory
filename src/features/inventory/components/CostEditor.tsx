  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        <label className="text-sm text-gray-600 w-40 shrink-0">Shipping in (€)</label>
        <input
      ...
      {/* Repair cost */}
      <div className="flex items-center gap-3">
        <label className="text-sm text-gray-600 w-40 shrink-0">Repair cost (€)</label>
        <input
        ...
        {fields.map((field, index) => (
          <div key={field.id} className="flex items-center gap-2">
            ...
            <input
              ...
              type="number" step="0.01" min="0"
              placeholder="€"
              className="w-24 border border-gray-300 rounded-md px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-gray-400"
            />
            <button
              type="button"
              onClick={() => remove(index)}
              className="text-gray-400 hover:text-red-600 text-sm transition-colors px-1"
            >
              ✕
            </button>
          </div>
      ...
      <button
        type="submit"
        disabled={isPending}
        className="bg-gray-900 text-white py-1.5 px-4 rounded-md text-sm font-medium hover:bg-gray-700 disabled:opacity-50 transition-colors"
      >

...
function PreStartBanner({ startsAt }: { startsAt: Date }) {
  return (
    <div className="absolute inset-0 rounded-2xl bg-indigo-50/50 border border-indigo-200 flex flex-col items-center justify-center gap-3 text-center px-6 backdrop-blur-[1px]">
      <span className="text-4xl">⏳</span>
      <p className="font-bold text-indigo-900 text-base drop-shadow-sm">
        Startet am{' '}
        {new Date(startsAt).toLocaleDateString('de-DE', {
          weekday: 'long', day: '2-digit', month: 'long', year: 'numeric',
        })}
      </p>
      <p className="text-sm text-indigo-700 drop-shadow-sm">
    ...
    <div className="space-y-8">
      ...
      {!isLoading && !isError && isNotStarted && (
        <div className="relative">
          <div className="grid grid-cols-3 gap-3 items-end pointer-events-none select-none">
          ...
          <PreStartBanner startsAt={result!.startsAt!} />
        </div>
      )}
